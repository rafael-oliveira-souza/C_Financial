...
input double  MOVIMENT_PERMANENT = 8;
...
bool verifyMagicNumber(int position = 0, ulong magicNumberRobot = 0){
   if(hasPositionOpen(position) || hasOrderOpen(OrderGetTicket(position))){
      ulong ticket = PositionGetTicket(position);
      PositionSelectByTicket(ticket);
      ulong magicNumber = PositionGetInteger(POSITION_MAGIC);
     ...
     // }
      ...
      if(USE_MAGIC_NUMBER == OFF){
         return true;
      }else if(magicNumber == magicNumberRobot || magicNumberOrder == magicNumberRobot){
         return true;
      }
   }
