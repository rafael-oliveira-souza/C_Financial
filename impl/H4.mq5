...
void toNegociate(double spread){
   if( CopyBuffer(handleVol[0], 0, 0, 5, VOL) == 5){
      ...
      if(VOL[0] <= VOL[1] && VOL[1] <= VOL[2] && VOL[2] <= VOL[3] && VOL[3] <= VOL[4]){
   ...
   for(int i = pos; i >= 0; i--)  {
      if(hasPositionOpen(i)){
         ...
         if(verifyMagicNumber(i, magicNumber)){
               closeBuyOrSell(i);
           /* if(PositionGetInteger(POSITION_TYPE) == type && profit < 0 && pointsProfit >= stop * 0.95 ){
               closeBuyOrSell(i);
...
void  moveTakeProfit( int position = 0){
   ...
   if(hasPositionOpen(position)){ 
      ...
      if(verifyMagicNumber(position, magicNumber)){
            ...
            if(profit > 0){
               if(pointsProfit >= pointsTake * 0.2 && pointsProfit <= pointsTake * 0.9){
                  ORIENTATION orientAverages = verifyOrientationAverage(currentPrice);
                  if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY && orientAverages != DOWN ){
                     executeOrderByRobots(UP, ACTIVE_VOLUME, PONTUATION_MOVE_STOP, pointsTake * 0.4);
                  }
                  else if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL && orientAverages != UP ){
                     executeOrderByRobots(DOWN, ACTIVE_VOLUME, PONTUATION_MOVE_STOP, pointsTake * 0.4);
                  }
              
                 if(pointsProfit >= pointsTake * 0.75 ){
           ...
           /*else if(profit < 0){
               if(pointsProfit >= pointsSL * 0.65){
               // && !hasTopOrBottom(12, TOP, currentPrice)
                  if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY){
                     executeOrderByRobots(UP, ACTIVE_VOLUME, 40, 10);
                  }
                  else if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL){
                     executeOrderByRobots(DOWN, ACTIVE_VOLUME, 40, 10);
                  }
              }
           }*/
       }
