   ...
   if(copiedPrice == 3){
      if( CopyBuffer(handleMd,0,0,3,averageMD) == 3 && CopyBuffer(handleMdd,0,0,3,averageMDD) == 3){
         if(hasPositionOpen() == false){
            double points = calcPoints(averageMD[2], averageMDD[2]);
            
            if( candles[2].close > bordersMediaRobot.max || candles[2].close < bordersMediaRobot.min){
               if(bordersMediaRobot.min != 0 && candles[2].close < bordersMediaRobot.min && calcPoints(bordersMediaRobot.min, candles[2].close) > 5){
                  orientationMacroMedia = DOWN;
               }else if(bordersMediaRobot.min != 0 && candles[2].close > bordersMediaRobot.max && calcPoints(bordersMediaRobot.max, candles[2].close) > 5){
            ...
            //if(( averageMD[1] < averageMDD[1] && averageMD[2] > averageMDD[2]) ||
               //( averageMD[1] > averageMDD[1] && averageMD[2] < averageMDD[2])){
                //waitNewCandleMedia = WAIT_CANDLES;
              //  closeBuyOrSell(0);
            //}
           if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL ){ 
               double entryDeal = PositionGetDouble(POSITION_PRICE_OPEN);
               if(averageMD[1] > averageMD[2] && averageMDD[1] < averageMDD[2] &&
                   averageMD[1] > averageMDD[1] && averageMD[2] < averageMDD[2]){
                   //waitNewCandleMedia = WAIT_CANDLES;
                   closeBuyOrSell(0);
               }
