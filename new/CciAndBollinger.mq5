...
void executeBearsAndBullsPower(MainCandles& mainCandles){
   if(CopyBuffer(handleBulls,0,0,periodAval,IBulls) == periodAval && CopyBuffer(handleBears,0,0,periodAval,IBears) == periodAval) {
      if(MathAbs(IBulls[penultimo] / IBears[penultimo]) > MAX_BULLS_AND_BEARS_VALUE ) {
         if((IBulls[penultimo]) > 0 && (IBears[penultimo]) < 0 && (IBulls[ultimo]) > 0 && (IBears[ultimo]) < 0) {
      ...
      else if(MathAbs(IBulls[penultimo] / IBears[penultimo]) > MAX_BULLS_AND_BEARS_VALUE ) {
         if((IBulls[penultimo]) < 0 && (IBears[penultimo]) > 0 && (IBulls[ultimo]) < 0 && (IBears[ultimo]) > 0) {
...
void executeIForce(MainCandles& mainCandles){
   if(CopyBuffer(handleIForce,0,0,periodAval,IFORCE) == periodAval) {
      double actualIdx = IFORCE[antipenultimo];
      double lastIdx = IFORCE[penultimo];
      double secondLastIdx = IFORCE[ultimo];
...
void executeBollingerBands(MainCandles& mainCandles){ 
   if(waitCandlesBollinger <= 0) {
      ...
      if(CopyBuffer(handleICCI,0,0,periodAval,CCI) == periodAval
         ...
         if(getCandleOrientantion(candles[primeiro]) == UP 
               && getCandleOrientantion(candles[segundo]) == UP 
               && candles[primeiro].close >= Upper[primeiro] 
               && candles[segundo].close >= Upper[segundo]
               && CCI[primeiro] >= MAX_CCI_VALUE
               && CCI[ultimo] <= MAX_CCI_VALUE) {
               ...
               if(!sellOrdersLocked) {
                  calibrateOrdersAndBuyOrSell(DOWN, stop, take);
                  waitCandlesBollinger = 1;
               }
            }               
         }
         else if(getCandleOrientantion(candles[primeiro]) == DOWN 
               && getCandleOrientantion(candles[segundo]) == DOWN 
               && candles[primeiro].close <= Lower[primeiro] 
               && candles[segundo].close <= Lower[segundo]
               && CCI[primeiro] <= -MAX_CCI_VALUE
               && CCI[ultimo] >= -MAX_CCI_VALUE) {
