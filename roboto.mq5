...
int MULTIPLIER_ROBOTS = 2, NUMBER_MAX_ROBOTS_ACTIVE = 0;
...
int OnInit(){
     ...
     // handleFI = iForce(_Symbol,TIME_FRAME, PERIOD_ROBOT ,MODE_SMA,VOLUME_TICK);
      ...
      ArrayResize(robots, NUMBER_MAX_ROBOTS_ACTIVE + 2);
      updateNumberRobots();
...
void updateNumberRobots(){
   ...
   if(EXPONENTIAL_VOLUME == ON){
     ...
     if(INITIAL_BALANCE > 0 && (BALANCE_ACTIVE+profit) / INITIAL_BALANCE >= MULTIPLIER_ROBOTS){
         MULTIPLIER_ROBOTS = MULTIPLIER_ROBOTS *2;
   ...
   if( CopyBuffer(handleVol, 0, 0, 2, VOL) == 2){
        if(VOL[0] >= VOL_OPERATION){
          // showComments(orientFI);
           double stop = STOP_LOSS * 4;
           double take = TAKE_PROFIT * 4;
...
void executeOrderByRobots(ORIENTATION orient, double volume, double stop, double take , bool order = false){
  if(countRobots < NUMBER_MAX_ROBOTS_ACTIVE){
...
void  decideToCreateOrDeleteRobots(){
   ...
   if(countBuy >= NUMBER_MAX_ROBOTS_ACTIVE * 0.5){
      verifyPos = true;
   }
   else if(countSell >= NUMBER_MAX_ROBOTS_ACTIVE * 0.5){
      verifyPos = true;
   }
   profitBuy = 0;
   profitSell = 0;
      ...
      if(hasPositionOpen(position)){
         ...
         if(verifyMagicNumber(position, magicNumber)){
            ...
            if(type == POSITION_TYPE_BUY ){ 
                  ...
                  if(profit > 0 ){
                     qtdBuyersWon++; 
                    // if(PONTUATION_MOVE_STOP < 100 && entryPriceToClose < 100  && verifyPos){
                      //  moveStopToZeroPlusPoint(position, 10);
                     //}
                     ...
                     if(MOVIMENT_PERMANENT > 0 && (MathAbs(profit/volume) > STOP_LOSS *0.6)){
  ...
  if(pos > 0){
     // double porcSell = (qtdSellersWon > 0 ? countSell / qtdSellersWon : 0) - (qtdSellersLoss > 0 ? countSell / qtdSellersLoss : 0);
      ...
      if(profitBuy  >= 0  ){
         ...
         if( lockSell == OFF){
            if(bestOrientation != UP && orientAverages != UP && orientationRVI == orientationMACD){
